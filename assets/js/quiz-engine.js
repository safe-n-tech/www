function initQuiz({ storageKey, questionsCount, filter, sentenceLevels }) {
  let questionIndex = 0;
  let questions = [];
  let results = null;
  ...
  async function startQuizz() {
    ...
    let allQuestions = (await response.json()).data;

    if (filter) {
      allQuestions = allQuestions.filter(filter);
    }

    questions = shuffle(allQuestions).slice(0, questionsCount);
  ...
  function showResults() {
    ...
    let sentenceLevel;
    if (results.score <= 25)      sentenceLevel = sentenceLevels[0];
    else if (results.score <= 50) sentenceLevel = sentenceLevels[1];
    else if (results.score <= 75) sentenceLevel = sentenceLevels[2];
    else                          sentenceLevel = sentenceLevels[3];
  ...
  function calculateResults() {
    questions.forEach(question => {
      ...
      question.choices.forEach(choice => {
        ...
        if (choice.text === question.userAnswer) question.isUserAnswerCorrect = true;
      });
    });

    const numberCorrect = questions.filter(q => q.isUserAnswerCorrect).length;
    const score = Math.trunc(numberCorrect * 100 / questions.length);

    let sentence;
    if (score <= 25)      sentence = "Vous êtes en danger";
    else if (score <= 50) sentence = "Vous êtes vulnérable";
    else if (score <= 75) sentence = "Vous êtes bon";
    else                  sentence = "Vous êtes parfait (ou presque) !";

    results = { numberCorrect, score, sentence, image: '' };
  }

  function createCorrection() {
    questions.forEach((question, index) => createElCorrection(question, index));
  ...
  function createElCorrection(question, questionIndex) {
    ...
    newCorrectionBtn.addEventListener("click", () => showCorrectionOfQuestionIndex(questionIndex));
  ...
  function showCorrectionOfQuestionIndex(questionIndex) {
    ...
    question.choices.forEach(choice => createElChoiceCorrection(choice, question));
  }
  ...
  function createElChoiceCorrection(choice, question) {
    ...
    if (choice.isCorrect) {
      if (choice.text === question.userAnswer) newChoiceCorrection.dataset.correctSelected = true;
      else                                     newChoiceCorrection.dataset.correct = true;
    } else {
      if (choice.text === question.userAnswer) newChoiceCorrection.dataset.wrongSelected = true;
    }
  ...
  function nextQuestion() {
    if (questions.userAnswer === null) return;
    questionIndex++;
    loadActualQuestion();
  }
  ...
  function resetQuestionAnswers() {
    Array.from(choicesContainer.children).forEach(child => { child.dataset.selected = false; });
  }
  ...
  function loadActualQuestion() {
    ...
    questions[questionIndex].choices.forEach(choice => createElChoice(choice));
  }
  ...
  function createElChoice(choice) {
    ...
    newChoice.addEventListener("click", () => handleAnswerSelected(newChoice));
  ...
  function saveQuizToLocalStorage() {
    localStorage.setItem(storageKey, JSON.stringify({ questions, questionIndex }));
  }
  ...
  function loadQuizFromLocalStorage() {
    const dataSavedJson = localStorage.getItem(storageKey);
    if (!dataSavedJson) return;

---
categories: category-vPOHpPFF_kFw2S80Eawqn
choices:
- isCorrect: false
  text: mettre ma date de naissance en mot de passe, c’est facile je la connais par
    cœur.
- isCorrect: false
  text: ne pas mettre de mot de passe, c’est plus rapide.
- isCorrect: true
  text: mettre un mot de passe complexe.
- isCorrect: true
  text: changer régulièrement le mot de passe.
correction: Si vous choisissez d’utiliser un mot de passe trop simple, ou de ne pas
  en mettre, vous risquez une utilisation de votre connexion internet par d’autres
  personnes à des fins malhonnêtes comme le téléchargement de contenu illégal (contenus
  piratés/ pédopornographiques) ou la diffusion de spam.
goodPractices:
- good-practice-A6hmiM-uR_gTzr7aPuTge
- good-practice-oo8d-eRVOqS8_S91fv-X-
text: 'Lorsque je fais un partage de connexion à un ami, j’ai l’habitude de :'
uuid: question-hHNqlm7YTQPlMTl3EpcgS
visibleInCms: true
---